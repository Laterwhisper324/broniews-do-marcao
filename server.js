// simple Express endpoint to receive cadastros and append to cadastros.txt
...
app.use(express.static(path.resolve(__dirname))); // serve static files (index.html, imagens...)

const CADASTROS_FILE = path.join(__dirname, 'cadastros.txt');

app.post('/cadastro', (req, res) => {
  const { name, email, password, sql } = req.body || {};
  if (!name || !email || !password || !sql) {
    return res.status(400).json({ ok: false, error: 'dados incompletos' });
  }

  const line = sql.replace(/\r?\n/g, ' ') + '\n';
